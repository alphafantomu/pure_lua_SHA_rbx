...
local unpack, table_concat, byte, char, string_rep, sub, gsub, gmatch, string_format, floor, ceil, math_min, math_max, tonumber, type, string_lower =
	table.unpack or unpack, table.concat, string.byte, string.char, string.rep, string.sub, string.gsub, string.gmatch, string.format, math.floor, math.ceil, math.min, math.max, tonumber, type, string.lower;
...
local is_LuaU = string_lower(_VERSION) == 'luau';
...
if print_debug_messages then
	...
	print("   Using LuaU:    "..(is_LuaU and "true" or "false"))
end
...
if branch == "FFI" or branch == "LJ" or branch == "LIB32" then
	...
	NORM = b.tobit or function(x)
		return x % 2^32;
	end;              -- only for LuaJIT
	HEX  = b.tohex or function(x, n)
		n = n or 8;
		local up;
		if (n <= 0) then
			if (n == 0) then 
